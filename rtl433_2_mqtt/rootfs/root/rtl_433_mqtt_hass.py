...
try:
    ...
    print("/config/config.yaml not found.  Reverting to defaults")
...
try:
    ...
    ids = config_file['discovery']['ids']
except:
    # Set defaults
    discovery = {'rtl_topic': 'rtl_433/+/events', 'discovery_prefix': 'homeassistant', 'force_update': False, 'discovery_interval': 600, 'expire_after': None, 'ids': None }
...
try:
    ## Need to provide some fallback defaults in case data is missing from config
    # Mappings data
...
# MQTT Config data
try:
    mqtt_config = config_file['mqtt']
except:
    logger.error("Failed to retrieve MQTT config")
...
try:
    SKIP_KEYS = config_file['skip_keys']
except:
...
def mqtt_connect(client, userdata, flags, rc):
    ...
    logger.info("MQTT connected: " + mqtt.connack_string(rc))
    if rc != 0:
        logger.error("Could not connect. Error: " + str(rc))
    else:
        logger.info("Subscribing to: " + discovery["rtl_topic"])
        client.subscribe(discovery["rtl_topic"])


def mqtt_disconnect(client, userdata, rc):
    """Callback for MQTT disconnects."""
    logger.info("MQTT disconnected: " + mqtt.connack_string(rc))


def mqtt_message(client, userdata, msg):
    """Callback for MQTT message PUBLISH."""
    logger.debug("MQTT message: " + json.dumps(msg.payload.decode()))
    ...
    try:
        ...
        logger.error("JSON decode error: " + msg.payload.decode())
        return
...
def rtl_433_device_info(data, topic_prefix):
    ...
    for match in re.finditer(TOPIC_PARSE_RE, mqtt_config["device_topic_suffix"]):
        path_elements.append(mqtt_config["device_topic_suffix"][last_match_end:match.start()])
        key = match.group(2)
        if key in data:
...
def publish_config(mqttc, topic, model, object_id, mapping, key=None):
    ...
    path = "/".join([discovery["discovery_prefix"], device_type, object_id, object_name, "config"])
    ...
    discovery_timeouts[path] = now + discovery["discovery_interval"]
    ...
    if discovery["force_update"]:
        config["force_update"] = "true"

    if discovery["expire_after"]:
        config["expire_after"] = discovery["expire_after"]

    logger.debug(path + ":" + json.dumps(config))

    mqttc.publish(path, json.dumps(config), retain=mqtt_config["retain"])
...
def bridge_event_to_hass(mqttc, topic_prefix, data):
    ...
    if "model" not in data:
        # not a device event
        logger.debug("Model is not defined. Not sending event to Home Assistant.")
        return
    ...
    if not device_id:
        ...
        logger.warning("No suitable identifier found for model: %s", model)
        return

    if ids and "id" in data and data.get("id") not in ids:
        # not in the safe list
        logger.info("Device (%s) is not in the desired list of device ids: %s" % (data["id"], ids))
        return
    ...
    if published_keys:
        logger.info("Published %s: %s" % (device_id, ", ".join(published_keys)))

        if skipped_keys:
            logger.info("Skipped %s: %s" % (device_id, ", ".join(skipped_keys)))
...
#        mqttc.enable_logger()
    ...
    mqttc.username_pw_set(mqtt_config["user"], mqtt_config["password"])
...
#        mqttc.tls_set(certfile=args.cert, keyfile=args.key, ca_certs=args.ca_cert)
    ...
    mqttc.connect_async(mqtt_config["host"], mqtt_config["port"], 60)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename='/config/rtl433_2_mqtt.log', format='[%(asctime)s] %(levelname)s:%(name)s:%(message)s',datefmt='%Y-%m-%dT%H:%M:%S%z', level=logging.WARNING)
    ...
    parser.add_argument("-l", "--log_level", type=str, dest="log_level", default="warning", help="Set the log level", action="store")
...
#        ids = ', '.join(str(id) for id in args.ids)
        logger.info("Only discovering devices with ids: %s" % ids)
    else:
