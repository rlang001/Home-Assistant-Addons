...
@app.route('/configuration/s_simulator', methods=['POST','GET'])
def s_simulator():
    # Read the file
    config = ConfigObj('/config/weewx.conf', list_values=False)
    ...
    if "Simulator" in config:
        try:
            loop_interval = config['Simulator']['loop_interval']
        except:
            loop_interval = 2.5
        ...
        try:
            start = config['Simulator']['start']
        except:
            start = ""

        simulator = {'Simulator': {'loop_interval': loop_interval, 'mode': mode, 'start': start, 'driver': 'weewx.drivers.simulator'}}
        config.merge(simulator)

    else:
        simulator = ConfigObj('/root/weewx-data/stations/simulator.conf', list_values=False)
        config.merge(simulator)
        config.write()
...
@app.route('/configuration/s_acurite', methods=['POST','GET'])
def s_acurite():

    return render_template('menu/config/stations/acurite.html')

@app.route('/configuration/s_cc300', methods=['POST','GET'])
def s_cc300():

    return render_template('menu/config/stations/cc300.html')
